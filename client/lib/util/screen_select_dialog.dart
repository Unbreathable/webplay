...
class ThumbnailWidget extends StatefulWidget {
  const ThumbnailWidget({super.key, required this.source, required this.selected, required this.onTap});
    ...
    return Column(
      children: [
        Expanded(
          ...
          child: InkWell(
            onTap: () {
              sendLog('Selected source id => ${widget.source.id}');
        ...
        Text(
          widget.source.name,
          style: TextStyle(fontSize: 12, color: Colors.black87, fontWeight: widget.selected ? FontWeight.bold : FontWeight.normal),
        ),
      ],
    );
  }
  ...
  Future<void> _getSources() async {
    try {
      ...
      for (var element in sources) {
        sendLog('name: ${element.name}, id: ${element.id}, type: ${element.type}');
      }
      _timer?.cancel();
